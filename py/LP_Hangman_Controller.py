...
class LP_Hangman_Controller(object):
    ...
    def showSentence(self):
        wordhints =self.mainView.get_all_word_hints()
        string = ''
        dummy = ',_'
        for word, length in zip(wordhints, self.currentSentenceWordLengths):
            string += word
            l = self.gem.translate_to_gematria(word)
            for i in range(length - len(l)):
                string += dummy
            string += ' '
        self.mainView.statusbar.showMessage(string)
    ...
    def handle_getSentence(self):
        ...
        self.currentSentenceWordLengths = []
        for word in self.currentSentence:
            self.currentSentenceWordLengths.append(len(word))
